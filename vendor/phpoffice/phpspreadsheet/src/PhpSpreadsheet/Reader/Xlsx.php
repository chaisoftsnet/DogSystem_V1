                            ...
                            if (!$this->readDataOnly && $xmlSheet && $xmlSheet->conditionalFormatting) {
                                ...
                                foreach ($conditionals as $ref => $cfRules) {
                                    ...
                                    foreach ($cfRules as $cfRule) {
                                        $objConnditional = new Conditional();
                                        $objConnditional->setConditionType((string) $cfRule['type']);
                                        $objConnditional->setOperatorType((string) $cfRule['operator']);

                                        if ((string) $cfRule['text'] != '') {
                                            $objConnditional->setText((string) $cfRule['text']);
                                        }

                                        if (isset($cfRule['stopIfTrue']) && (int) $cfRule['stopIfTrue'] === 1) {
                                            $objConnditional->setStopIfTrue(true);
                                        }

                                        if (count($cfRule->formula) > 1) {
                                            foreach ($cfRule->formula as $formula) {
                                                $objConnditional->addCondition((string) $formula);
                                            }
                                        } else {
                                            $objConnditional->addCondition((string) $cfRule->formula);
                                        }
                                        $objConnditional->setStyle(clone $dxfs[(int) ($cfRule['dxfId'])]);
                                        $conditionalStyles[] = $objConnditional;
                                    }
